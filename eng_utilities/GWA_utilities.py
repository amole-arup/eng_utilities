...
def write_GWA(E2K_dict, GWApath, GSA_ver=10, add_poly=False, debug=False): # save_pickle=False,
    ...
    with open(GWApath, 'w', encoding='utf8') as gwa:
        ...
        E2K_dict['GWA_dicts'] = {}
        GWA_dicts = E2K_dict.get('GWA_dicts',{})
        
        GWA_dicts['E2K_MAT_type_map'] = {
        ...
        ETABS2GWA_mat_type_dict = GWA_dicts['E2K_MAT_type_map']
        if debug: print(f'Length of mat_type_dict is {len(ETABS2GWA_mat_type_dict)}')

        GWA_dicts['Design_MAT_lookup'] = {}
        if debug: print(f'Length of design_mat_lookup is {len(GWA_dicts["Design_MAT_lookup"])}')


        def write_GWA_mat(E2K_dict, gwa, grav, debug=False):
            
            MAT_PROPS_dict = E2K_dict.get('MATERIAL PROPERTIES',{}).get('MATERIAL',{})
            design_mat_lookup = E2K_dict.get('GWA_dicts',{}).get('Design_MAT_lookup',{})
            ...
            for name, mat_dict in MAT_PROPS_dict.items():
                ...
                if mat_type.casefold().startswith('conc'):
                    ...
                    gwa.write(GWA_mat_string(name, mat_dict, num=conc_ID, grav=grav, debug=debug) + '\n')
                elif mat_type.casefold() == 'steel':    
                    steel_ID += 1
                    design_mat_lookup[m_ID] = str(steel_ID)
                    gwa.write(GWA_mat_string(name, mat_dict, num=steel_ID, grav=grav, debug=debug) + '\n')
                else:
                    design_mat_lookup[m_ID] = ''
            
            if debug: 
                print(f'Length of design_mat_lookup (int) is {len(design_mat_lookup)}')
                print(f"Length of design_mat_lookup (int) is {len(E2K_dict.get('GWA_dicts',{}).get('Design_MAT_lookup',{}))}")

        write_GWA_mat(E2K_dict, gwa, grav, debug=debug)

        design_mat_lookup = GWA_dicts['Design_MAT_lookup']
        if debug: print(f'Length of design_mat_lookup (ext) is {len(design_mat_lookup)}')
        ...
        # member type: BEAM, COLUMN, GENERIC_1D, SLAB, WALL, GENERIC_2D, 1D_VOID_CUTTER, 2D_VOID_CUTTER
        ...
        for sh_name, sh_dict in AREA_dict.items():
            ...
            if num_pts > 2:
                ...
                if sh_dict.get('OPENING', '') == 'YES':
                    el_type = '2D_VOID_CUTTER'  # NOT `VOID_CUTTER_2D``
